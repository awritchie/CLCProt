-- Binding Variables
BINDING_HEADER_CLCPROT = "CLCProt Rotation Helper";

local RotationSet = 1
function SetRotation(n) 
	RotationSet = n
end
function GetRotationSet()
	return RotationSet
end
...
local function CmdLinePrio(args)
	clcprot.db.profile.rotation.prio = args
...
function clcprot:QUEST_LOG_UPDATE()
	...
	self:RegisterChatCommand("clcprotlp", CmdLinePrio)
...
-- ---------------------------------------------------------------------------------------------------------------------
-- holy blank function
function clcprot:DoNothing()
	self:CLCPROTDisable()
end



function clcprot:CheckQueueRet()
	dq[1], dq[2] = clcprot.RetRotation()
...
-- ENABLE/DISABLE
-- ---------------------------------------------------------------------------------------------------------------------
function clcprot:CLCPROTEnable()
	if addonInit then
		addonEnabled = true
		self.frame:Show()
	end
...
function clcprot:CLCPROTDisable()
	if addonInit then
		addonEnabled = false
		self.frame:Hide()
	end
...
function clcprot:COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, combatEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellId, spellName, spellSchool, spellType, dose, ...)
	-- pass info for the sov function
	if db.sov.enabled then
		clcprot:SOV_COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, combatEvent, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellId, spellName, spellSchool, spellType, dose, ...)
	end
